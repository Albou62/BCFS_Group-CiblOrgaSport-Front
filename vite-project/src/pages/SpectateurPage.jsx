...
function SpectateurPage({ token, username, onLogout }) {
  ...
  const fetchHello = async () => {
    try {
      const res = await fetch(`${AUTH_API_URL}/api/hello`, {
  ...
  const fetchUpcomingEpreuves = async () => {
    try {
      const res = await fetch(`${COMP_API_URL}/api/public/upcoming-epreuves`);
      if (!res.ok) throw new Error('Erreur API compétitions');
      const data = await res.json();
      // data doit être une liste d'épreuves/compétitions depuis ton back
      setEvents(data);
    } catch (err) {
      console.error(err);
    }
        ...
        <div className="spectator-main">
          ...
          <div className="panel">
            ...
            <p className="panel-subtitle">
              Épreuves à venir exposées par le service compétitions.
            </p>
              ...
              {events.map((e) => (
                <li key={e.id} className="event-item">
                  ...
                  <div className="event-meta">
                    {e.venue || e.lieu || 'Lieu communiqué ultérieurement'}
                  </div>
                  <div className="event-meta">
                    {e.date || e.horaire_public || 'Date à confirmer'}
                  </div>
                </li>
              ))}
              {events.length === 0 && (
