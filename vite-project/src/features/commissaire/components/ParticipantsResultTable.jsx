...
function ParticipantsResultTable({ participants, label, onResultChange, onToggleStatus }) {
  return (
    <table style={{ marginBottom: '1.5rem', width: '100%' }}>
      ...
      <tbody>
        {participants.map((p) => (
          <tr key={p.athleteId} style={{ borderBottom: '1px solid #e2e8f0', background: p.statut === 'DQ' ? '#fee2e2' : 'white' }}>
            ...
            <td style={{ padding: '12px' }}>
              <input
                type="text"
                value={p.resultat}
                onChange={(e) => onResultChange(p.athleteId, e.target.value)}
                placeholder="0.00"
            ...
            <td style={{ padding: '12px', textAlign: 'center' }}>
              <button
                onClick={() => onToggleStatus(p.athleteId)}
                style={{
                  padding: '6px 12px',
                  borderRadius: '6px',
                  cursor: 'pointer',
